...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor(private alertService: AlertService, private eventManager: EventManager, translateService: TranslateService) {
    this.errorListener = eventManager.subscribe('oneTimaCapsuleApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = eventManager.subscribe('oneTimaCapsuleApp.httpError', (response: EventWithContent<unknown> | string) => {
        ...
        case 400: {
          ...
          if (errorHeader) {
            ...
            for (const fieldError of fieldErrors) {
              ...
              const fieldName: string = translateService.instant(`oneTimaCapsuleApp.${fieldError.objectName as string}.${convertedField}`);

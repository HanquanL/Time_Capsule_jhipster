...
describe('Alert Error Component', () => {
  ...
  describe('Error Handling', () => {
    it('Should display an alert on status 0', () => {
      // GIVEN
      eventManager.broadcast({ name: 'oneTimaCapsuleApp.httpError', content: { status: 0 } });
      // THEN
    ...
    it('Should display an alert on status 404', () => {
      // GIVEN
      eventManager.broadcast({ name: 'oneTimaCapsuleApp.httpError', content: { status: 404 } });
      // THEN
      expect(comp.alerts.length).toBe(1);
    ...
    it('Should display an alert on generic error', () => {
      // GIVEN
      eventManager.broadcast({ name: 'oneTimaCapsuleApp.httpError', content: { error: { message: 'Error Message' } } });
      eventManager.broadcast({ name: 'oneTimaCapsuleApp.httpError', content: { error: 'Second Error Message' } });
      // THEN
    ...
    it('Should display an alert on status 400 for generic error', () => {
      ...
      eventManager.broadcast({ name: 'oneTimaCapsuleApp.httpError', content: response });
      // THEN
      expect(comp.alerts.length).toBe(1);

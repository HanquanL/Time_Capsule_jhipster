...
/**
 * REST controller for managing {@link com.onetimecapsule.app.domain.Post}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Post> partialUpdatePost(
        ...
        Optional<Post> result = postRepository
            ...
            .map(existingPost -> {
                ...
                if (post.getDescrption() != null) {
                    existingPost.setDescrption(post.getDescrption());
                }
                if (post.getVotecount() != null) {
                    existingPost.setVotecount(post.getVotecount());
                }
